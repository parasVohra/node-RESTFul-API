...
// Checks - post
// Required data: protocol, url, methods, successCodes, timeoutSecond
  ...
  if (id) {
    ...
    _data.read("checks", id, function (err, checkData) {
      if (!err && checkData) {
        ...
        //Verify that the give token is valid  and belong to the user who create the check
                      ...
                      if (!err && userData) {
                        let userChecks =
                          ...
                          userData.checks instanceof Array
                            ? userData.checks
                            : [];
                        // Remove the deleted check from their list of check
                        ...
                        if (checkPosition > -1) {
                          ...
                          callback(500, {
                            Error:
                              "Could not find the check on the users object, so could not delete the check",
                          });
                        }
                      } else {
                        callback(500, {
                          Error:
                            "Could not find user who create the check, so could not delete the check from the user object ",
                        });
                      }
                    }
                  );
