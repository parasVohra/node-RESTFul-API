...
// Append a string to a file. Crete the file if it does not exits.
...
// Compress the contents of one .log file into the same directory
...
lib.decompress = function (fieldId, callback) {
  ...
  fs.readFile(lib.baseDir + fileName, "utf-8", function (err, str) {
    if (!err && str) {
      // Decompress the data
      let inputBuffer = new Buffer.from(str, "base64");
